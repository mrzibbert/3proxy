...
apt-get update && apt-get install gcc make wget nano tar gzip -y && apt-get install  --reinstall build-essential
wget --no-check-certificate -O 3proxy-${version}.tar.gz https://github.com/z3APA3A/3proxy/archive/${version}.tar.gz
tar zxvf 3proxy-${version}.tar.gz
cd 3proxy-${version}
...
cd /etc/3proxy/
wget --no-check-certificate https://github.com/mrzibbert/3proxy/raw/main/3proxy.cfg
